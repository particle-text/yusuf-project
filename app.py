...
query = st.text_input("اكتب اسم العنصر (عربي / إنجليزي / الرمز)")
...
if query:
    ...
    for el in elements.values():
        names = [
            normalize(el.get("name_ar", "")),
            normalize(el.get("name_en", "")),
...
if query:
    if found:
        ...
        st.write(f"**الاسم بالعربي:** {found.get('name_ar', '-')}")
        st.write(f"**الاسم بالإنجليزي:** {found.get('name_en', '-')}")
        st.write(f"**الرمز:** {found.get('symbol', '-')}")
        st.write(f"**العدد الذري:** {found.get('atomic_number', '-')}")
        ...
        st.write(f"**الخصائص:** {found.get('properties', '-')}")
        st.write(f"**موقعه في الطبيعة:** {found.get('nature', '-')}")
    else:
